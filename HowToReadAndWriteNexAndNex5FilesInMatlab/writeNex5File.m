...
% Edited by ZL on 2019-11-24 to make the output file identical to the OFS exported file
    % based on the Oct-26-2019 release of 'HowToReadAndWriteNexAndNex5FilesInMatlab' from https://www.neuroexplorer.com/downloadspage/
...
%         s = sprintf('{"name":"%s","probe":{"wireNumber":%d},"unitNumber":%d},',nexFile.neurons{i}.name, nexFile.neurons{i}.wireNumber,nexFile.neurons{i}.unitNumber);
        s = sprintf('{"name":"%s","probe":{"wireNumber":%d},"unitNumber":%d},',...
            deblank(nexFile.neurons{i}.name), nexFile.neurons{i}.wireNumber,nexFile.neurons{i}.unitNumber);  % remove ending whitespace
        meta = strcat(meta,s);
    end
end
...
for i = 1:waveCount
    ...
    if writeContVarsAsFloats == 1
        ...
        if varHeader.Count > 0
			...
			if ~isfield(nexFile.waves{i}, 'ADtoMV')   % ZL 2019-11-24
			...
			nexFile.waves{i}.ADtoMV = c;  % only calculate 'ADtoMV' when it is not available
			end
        end
    end
...
%         s = sprintf('{"name":"%s","probe":{"wireNumber":%d},"unitNumber":%d},',nexFile.waves{i}.name, nexFile.waves{i}.wireNumber,nexFile.waves{i}.unitNumber);
        s = sprintf('{"name":"%s","probe":{"wireNumber":%d},"unitNumber":%d},',...
            deblank(nexFile.waves{i}.name), nexFile.waves{i}.wireNumber,nexFile.waves{i}.unitNumber);  % remove ending whitespace
        meta = strcat(meta,s);
    end
end
...
for i = 1:contCount
    ...
    if writeContVarsAsFloats == 1
        ...
        c = 1;
        if varHeader.Count > 0
			if ~isfield(nexFile.contvars{i}, 'ADtoMV')   % ZL 2019-11-24
            % we need to recalculate a/d to milliVolts factor
			...
			nexFile.contvars{i}.ADtoMV = c;  % only calculate 'ADtoMV' when it is not available
			end
        end
    end
